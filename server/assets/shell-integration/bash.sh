...
__dh_debug() {
  ...
  # BASH_COMMAND is not lossless for leading whitespace, compound, multiline,
  # substitution, or redirection syntax. The simple-command path intentionally
  # records Bash's normalized command text; ambiguous syntax is rejected below.
  case "$command" in
    ""|*'&&'*|*'||'*|*';'*|*'|'*|*'<'*|*'>'*|*'$('*|*'`'*|*'{'*|*'}'*|*'('*|*')'*|*$'\n'*)
