...
use super::registry_journal::{list_journal_records, write_journal_record, TransactionPhase};
    ...
    for mut record in records {
        ...
        match record.phase {
                ...
                let should_clean = if stage_review_file.exists() {
                    if let Ok(content) = fs::read_to_string(&stage_review_file) {
                        if let Ok(review) = serde_json::from_str::<serde_json::Value>(&content) {
                            if let Some(exp_str) =
                                review.get("stageExpiresAt").and_then(|v| v.as_str())
                            {
            ...
            TransactionPhase::Approved | TransactionPhase::Extracted => {
                ...
                record.error =
                    Some("Transaction interrupted before registry registration".to_string());
