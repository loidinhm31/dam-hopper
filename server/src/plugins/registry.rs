...
impl PluginRegistry {
    pub fn new(layout: PluginRegistryLayout, admin_subjects: AdminSubjectList) -> Result<Self, PluginError> {
        ...
        if !registry_file.exists() {
            ...
            let bytes = serde_json::to_vec_pretty(&initial).map_err(|e| {
                PluginError::runner_unavailable(format!("Failed to serialize initial registry: {e}"))
            })?;
            crate::linux_release::durable_fs::atomic_write_file(&registry_file, &bytes, Some(0o600)).map_err(|e| {
    ...
    pub fn write_state(&self, record: &RegistryV1Record) -> Result<(), PluginError> {
        ...
        crate::linux_release::durable_fs::atomic_write_file(&path, &bytes, Some(0o600)).map_err(|e| {
            PluginError::runner_unavailable(format!("Failed to write registry: {e}"))
        })?;
