...
async fn write_binary_happy_path() {
    ...
    let ack = next_json(&mut ws, Duration::from_secs(5))
        .await
        .expect("fs:write_ack");
    ...
    .await
    .unwrap();
    ws.send(Message::Binary(new_content.to_vec().into()))
        .await
        .unwrap();

    let chunk_ack = next_json(&mut ws, Duration::from_secs(5))
        .await
        .expect("fs:write_chunk_ack");
...
async fn write_binary_occ_conflict() {
    ...
    .await
    .unwrap();
    ws.send(Message::Binary(b"chunk".to_vec().into()))
        .await
        .unwrap();
