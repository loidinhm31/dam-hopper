...
- **Safe Inline Suggestions**: supported local interactive zsh/fish sessions expose only a server-validated lifecycle to a fail-closed, per-terminal controller. Ghost acceptance writes a suffix only; unsupported shells, replay, alternate buffers, and mobile input show no automatic UI. Exact command history stays browser-local with clear and disable controls.
