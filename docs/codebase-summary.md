...
- **Session-Bound Media**: `server/src/fs/media_ticket.rs` centrally tracks opaque video/image tickets and actor-bound media sessions; streams require the matching `HttpOnly` media cookie, ticket/session expiry is bounded (30-minute idle, eight-hour absolute), and session revocation invalidates its tickets. `ServerSettingsDialog` bounds remote revocation to five seconds during credential transitions/logout: unreachable servers fall back to expiry, while a successful remote revoke is intentionally not undone if later local token persistence/removal fails, so media must be reissued.
