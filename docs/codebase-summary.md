...
- **Terminal Usage Analytics**: `server/src/telemetry/` provides an opt-in private SQLite store, WAL/SHM permission hardening, idempotent event writes, a bounded 100-item/250ms worker, UTC rollup-before-purge retention, HMAC fingerprints, project exclusions, loopback Codex OTLP ingestion, and aggregate-only query APIs. Startup/shutdown and SQLite faults remain isolated from PTY operation; `NoopTelemetrySink` is the disabled fallback. `packages/ui/src/components/pages/UsagePage.tsx` exposes coverage, pause, exclusion, retention, and explicit deletion controls.
