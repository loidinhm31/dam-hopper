...
- Repomix v1.18.0 snapshot: 1,142 packed files, 2,058,224 tokens, 8,028,681 characters
- Predominantly Rust (server) and TypeScript/React
- Compaction includes source, docs, plans, and configuration; binary/default-ignored files are omitted

## Semantic LSP Phase 2 Foundation

`server/src/semantic/` is a backend-only, server-owned runtime boundary. `AppState`
constructs `SemanticRegistry` and `SemanticSupervisor` with a fixed bundle root at
`<server-executable-parent>/semantic-bundles`; missing or invalid release inputs
leave capability unavailable and never probe PATH or project commands. The resolver
checks manifest schema/target, detached Ed25519 signature, manifest SHA-256,
executable mode/root containment/size, and artifact SHA-256 before direct child
spawn. Rust uses `rust-analyzer`; JavaScript and TypeScript have distinct logical
IDs but share the fixed root `typescript-language-server --stdio` command and Node
runtime. Java is registered but disabled until Phase 6.

The child startup path sends bounded JSON-RPC `initialize`, waits up to 2 seconds
for the matching response, validates at most 128 capabilities, then sends
`initialized`. Sessions cap 2 active requests, 32 queued, and 16 MiB queued-frame
bytes; each frame is capped at 8 MiB and each stdin write at 100 ms. Session keys
include client, project, descriptor fingerprint, and trust-policy revision. Trust
is persisted server-side when possible and defaults restricted; trusted only enables
reviewed server-owned initialization deltas and is not an OS sandbox. Generation
fencing, dispatch serialization, revocation teardown, 10-minute idle/60-second
sweep, and one-idle-session pressure eviction protect lifecycle admission. Phase
3 adds the authenticated semantic WebSocket, bounded full-snapshot document
sync/replay, cancellation-aware navigation, project-relative result mapping, and
trust/workspace lifecycle fencing. Unsaved snapshots remain connection-scoped.
...
│   │   ├── semantic/         # Bundled LSP registry/supervisor/runtime
...
**Phase Status**: Semantic LSP Phase 3 transport and document/navigation synchronization completed; release qualification and later UX remain outside this boundary
**Generated by**: Repomix v1.18.0 compaction + manual documentation
