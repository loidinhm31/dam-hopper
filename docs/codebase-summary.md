...
- **Terminal Usage Analytics (Phase 03)**: `server/src/telemetry/` provides an opt-in private SQLite store, WAL/SHM permission hardening, idempotent event writes, a bounded 100-item/250ms worker, UTC rollup-before-purge retention, HMAC fingerprints, project exclusions, and read-only aggregate query foundations. Startup/shutdown keeps telemetry failures isolated from PTY operation; `NoopTelemetrySink` remains the disabled fallback. Usage API routes remain Phase 04.
