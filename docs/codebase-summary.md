...
- **HTML Preview**: `packages/ui/src/components/organisms/HtmlPreview.tsx`, `packages/ui/src/components/organisms/HtmlHost.tsx`, `packages/ui/src/components/organisms/EditorTabs.tsx`, `packages/ui/src/lib/html-file.ts`, and `packages/ui/src/lib/html-view-mode-persistence.ts` provide sandboxed iframe rendering (`sandbox="allow-scripts allow-modals"` without `allow-same-origin`), 200ms debouncing, local view mode persistence (`dam-hopper:html-view-mode:v1`, default `"edit"`), `HtmlHost` with Edit | Split | Preview mode switching, and lazy routing in `EditorTabs` for `.html`, `.htm`, and `.xhtml` files.
