...
- **HTML Preview**: `packages/ui/src/components/organisms/HtmlPreview.tsx`, `packages/ui/src/lib/html-file.ts`, and `packages/ui/src/lib/html-view-mode-persistence.ts` provide sandboxed iframe rendering (`sandbox="allow-scripts allow-modals"` without `allow-same-origin`), 200ms debouncing, and local view mode persistence (`dam-hopper:html-view-mode:v1`, default `"edit"`) for `.html`, `.htm`, and `.xhtml` files.
