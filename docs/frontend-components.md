...
4. `terminal-agent-notification-integration.ts` stores each accepted event in the bounded in-memory Zustand notification store, attempts the short in-app chime, and forwards the same event to `BrowserNotificationService`.
