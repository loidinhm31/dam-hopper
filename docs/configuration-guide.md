...
2. **Release-manager / systemd production**: `/etc/dam-hopper/dam-hopper.toml`, as configured and passed by `deploy/systemd/dam-hopper-api.service`.
...
2. In `/etc/dam-hopper/dam-hopper.toml`, set `automatic_policy = "empty-fleet"`.
...
1. In `/etc/dam-hopper/dam-hopper.toml`, set `enabled = false` and `automatic_policy = "empty-fleet"`.
