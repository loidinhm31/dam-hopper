...
- **Persistent Explorer Tree expansion store (Phase 01).** Added persistent Zustand store `useExplorerTreeStore` to preserve directory open/closed states across sidebar tool switching, shell layout changes (IDE vs. Terminal workspace modes), sidebar collapsing/reopening, and browser page reloads.
- Target scoping: Isolated per project and target/worktree via `explorerTreeScopeKey(target)` (`${normalized.project}::${projectTargetCacheKey(normalized)}`) and persisted to `localStorage` under `dam-hopper:explorer-tree-state`.
- Integration: `FileTree` connects via `initialOpenState` and `onToggle` updates; remounting with persisted expansion states triggers automatic cascading child loads for unloaded directories, safely pruning missing or deleted directories from the store if loading fails.
- Tree mutations: Directory renames, drag-and-drop moves, and deletions synchronize with `renamePath` and `prunePath` store actions so exact paths and descendant subtree keys stay consistent without stale keys.
- Focused validation passed: 8 unit tests in `explorer-tree.test.ts`, UI TypeScript build (`pnpm --filter @dam-hopper/ui build`), and file-tree helper tests (`file-tree-reveal.test.ts`, `tree-copy-paths.test.ts`). [See plan](../plans/260831-1802-preserve-explorer-tree-and-editor-scroll/phase-01-explorer-tree-expansion-store.md).
