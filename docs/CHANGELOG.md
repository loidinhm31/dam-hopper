...
- **Linux Release Installer Architecture — Phase 09: Documentation, Roadmap, Changelog, and Release Cutover [COMPLETED 2026-09-04].** Cut maintained documentation over to the attested Fedora 44 x86_64 release installer: bootstrap → pending install → explicit `dam-hopper start`, health-gated rollback/recovery, role/origin contracts, and format-2 migration guidance. Obsolete checkout-runner instructions were removed from maintained operator docs or retained only as clearly historical context.
  - **Breaking Changes:** Retired checkout-built production runner (`deploy/run-linux-production.sh`, `deploy/reset-linux-production.sh`, fixed `deploy/systemd/dam-hopper.service`, fixture `tests/deploy/linux-production-fixtures.sh`, and package aliases `pnpm linux:production` / `pnpm linux:reset`).
  - **Authoritative Docs:** Rewrote `docs/linux-systemd.md` in place under 800 lines as the comprehensive Fedora 44 x86_64 operator contract; updated `README.md` with bootstrap installer quickstart; reconciled configuration, profile, API reference, architecture, standards, and codebase summary docs.
  - **Support & Security Boundary:** Documents API `0.0.0.0:4801` (running as `root` for MVP PTY operations), dedicated web host `0.0.0.0:4802` (`dam-hopper-web` unprivileged sysuser), mandatory SHA-256 integrity checks, optional GitHub attestation verification via `gh`, and atomic format-2 migration via Linux `renameat2(RENAME_EXCHANGE)`. Machine-local runtime values, credentials, and SQLite databases remain strictly excluded from release archives and are never mutated or purged.
  - Validation: maintained-document links (145 relative links, 18 anchors), Markdown structure, CLI syntax against implemented `--help`, docs line count (`docs/linux-systemd.md`: 313 lines), and stale runner-reference audit passed; docs-manager scope review passed ([docs report](../plans/reports/docs-manager-260904-1601-phase-09-documentation-release-cutover.md)). Completion is grounded in Phase 08 terminal tester/reviewer evidence on Fedora 44 x86_64/systemd 259 ([tester](../plans/reports/tester-260904-1316-phase-08-behavioral-security-failure-validation.md), [review](../plans/reports/code-review-260904-1320-phase-08.md)). External MongoDB, firewall/Tailscale, DNS/TLS automation, unsupported distros, and protected-tag publication remain outside this cutover. Parent plan progress: **100% (136h/136h; Phases 01–09 complete)**. [See Phase 09 plan](../plans/260903-0919-linux-release-installer-architecture/phase-09-documentation-roadmap-changelog-cutover.md).
...
- Validation: all 8 verification commands passed; terminal tester report verified zero failures; code review approved 9.8/10 with all findings remediated. Parent plan progress: **100% (136h/136h); Phase 09 completed**. [See Phase 08 plan](../plans/260903-0919-linux-release-installer-architecture/phase-08-behavioral-security-failure-validation.md).
...
- Validation: vendored all-target `cargo check` passed; focused manifest suites passed 20/20 and the `linux_release` filter passed 10/10 (30/30 executed test invocations); scoped review approved 9.5/10 with no blocking findings. At that Phase 01 milestone, the parent plan was 7% complete (10h/136h) and Phases 02–09 were pending. [See plan](../plans/260903-0919-linux-release-installer-architecture/plan.md).
- **Linux Release Installer Architecture — Phase 02: Rust CLI and Safe Acquisition/Staging [COMPLETED 2026-09-03].** Delivered the Rust manager CLI and privilege matrix, Fedora 44/x86_64/systemd profile validation, bounded GitHub acquisition, mandatory SHA-256 verification with optional attestation, root-safe staging with source-integrity checks, strict archive traversal, role projections, and durable pending metadata without service activation.
- Validation: `cargo check --manifest-path server/Cargo.toml --bin dam-hopper --tests` passed; `linux_release_*` suites passed **45/45 across 7 suites**; scoped clippy and `systemctl --version` checks passed; code review approved **8.8/10** with no blocking findings. Non-blocking hardening recommendations remain recorded in the [review report](../plans/reports/code-review-260903-1644-phase-02-rust-cli-acquisition-staging.md). Parent plan is **21% complete (28h/136h)**; Phases 03–09 remain pending. [See plan](../plans/260903-0919-linux-release-installer-architecture/phase-02-rust-cli-safe-acquisition-staging.md).
- **Linux Release Installer Architecture — Phase 03: Dedicated Web Host, Runtime Origin, and Health [COMPLETED 2026-09-03].** Added the non-writing `dam-hopper-web` static host on port `4802`, reserved `/__dam-hopper/health` and `/__dam-hopper/runtime-config.json` routes, strict symlink/traversal-safe SPA serving, deterministic cache headers, bounded runtime-origin validation, and managed browser profile reconciliation. The API is API-only by default; Docker retains combined serving through explicit `--web-dir /opt/dam-hopper/web`.
- Validation: `linux_release_web_host` passed **8/8**; API-only/default and API-health focused tests passed **2/2**; runtime/server-config Vitest passed **72/72**; vendored all-target Cargo check plus UI/web builds exited `0`; code review approved **8.5/10** with no blocking findings. Parent plan is **31% complete (42h/136h)**. [See phase](../plans/260903-0919-linux-release-installer-architecture/phase-03-web-host-runtime-origin-health.md).
- **Linux Release Installer Architecture — Phase 04: Role-Aware systemd Units and Ownership [COMPLETED 2026-09-03 20:57:20 +07:00].** Delivered independent API/web systemd unit templates, strict placeholder rendering and policy validation, exact role inventory, dedicated `dam-hopper-web` sysuser, ownership/mode and process/listener evidence, root-private pending-unit staging, atomic host configuration, and no-shell systemd adapters. API runs as `root` per the accepted MVP directive; web remains separately unprivileged and isolated.
- Validation: Linux release integration suites passed **66/66 across 10 suites**; vendored all-target `cargo check` passed with no warnings; scoped review approved **9.0/10** with no blocking findings. Non-blocking hardening recommendations remain recorded in the [review report](../plans/reports/code-review-260903-2025-phase-04-role-aware-systemd-ownership.md). Parent plan is **40% complete (54h/136h)**; Phases 05–09 remain pending. [See phase](../plans/260903-0919-linux-release-installer-architecture/phase-04-role-aware-systemd-ownership.md).
