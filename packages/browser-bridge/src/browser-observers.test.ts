...
describe("Browser observers", () => {
  ...
  it("redacts credential-like console values and reports uncaught errors", () => {
    ...
    const stop = observeConsole((level, message) => entries.push({ level, message }));
    ...
    window.dispatchEvent(new ErrorEvent("error", { message: "Unhandled failure" }));
    stop();

    expect(
      entries.filter(
        (entry) =>
          entry.level === "info" && entry.message === "Authorization=[REDACTED]",
      ),
    ).toHaveLength(2);
