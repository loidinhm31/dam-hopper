...
/**
 * Build a stable OPAQUE identifier scoped to this client session.
 * The identifier is deterministic per (project, passphrase-session).
 */
function buildIdentifier(project: string): string {
  // Simple alphanumeric slug: "enc-<project>-<random-suffix>"
  // The random suffix is a session-local nonce generated at hook mount.
  // It is NOT persisted — each page load generates a new identifier,
  // ensuring the in-memory ephemeral model stays fresh.
...
export function useEncryptedWrite(): UseEncryptedWriteReturn {
  ...
  // Cache the session per project to avoid re-authenticating for each save
  const sessionCacheRef = useRef<Map<string, OpaqueSessionResult>>(new Map());

  const getOrCreateSession = useCallback(
    async (project: string, passphrase: string): Promise<OpaqueSessionResult> => {
      const cached = sessionCacheRef.current.get(project);
      if (cached) return cached;

      setStatus("authenticating");
      const transport = getTransport() as WsTransport;
      const identifier = buildIdentifier(project);
      const session = await opaqueRegisterAndLogin(transport, identifier, passphrase);
      sessionCacheRef.current.set(project, session);
      return session;
    },
    [],
  );
  ...
  const uploadFile = useCallback(
    async (
      ...
      try {
        ...
        setStatus("error");
        setError(msg);
        // Invalidate session cache on error (may be auth failure)
  ...
  const saveText = useCallback(
    async (
      ...
      try {
        ...
        setStatus("error");
        setError(msg);
        // Invalidate session cache on error
        sessionCacheRef.current.clear();
        return { ok: false, error: msg };
      }
    },
