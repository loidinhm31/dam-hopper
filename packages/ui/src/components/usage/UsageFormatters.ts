...
export function formatCompactUsageNumber(value: number | null | undefined): string {
  return typeof value === "number" && Number.isFinite(value) ? compactNumberFormatter.format(value) : "—";
}
...
export function formatPercent(numerator: number | null | undefined, denominator: number | null | undefined): string {
...
export function hasTokenTotal(tokens: UsageTokens | null | undefined): boolean {
  ...
  return [tokens.inputTokens, tokens.outputTokens, tokens.reasoningTokens].some((value) => value !== null && value !== undefined);
}
...
export function formatTokenTotal(tokens: UsageTokens | null | undefined): string {
  if (!tokens || !hasTokenTotal(tokens)) return "—";
  return formatCompactUsageNumber([tokens.inputTokens, tokens.outputTokens, tokens.reasoningTokens].reduce<number>((sum, value) => sum + (value ?? 0), 0));
}
...
export function formatUsageBucketLabel(startUtcMs: number, bucket: "hour" | "day"): string {
  const date = new Date(startUtcMs);
  return new Intl.DateTimeFormat(undefined, { month: "short", day: "numeric", ...(bucket === "hour" ? { hour: "numeric" as const } : {}) }).format(date);
}
