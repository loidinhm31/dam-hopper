...
function checkDisallowedPath(relPath) {
  ...
  if (DISALLOWED_FILES.has(fileName) || fileName.startsWith('.env.')) {
    throw new Error(`Prohibited runtime configuration file in release archive: ${relPath}`);
  }
...
function parseArgs() {
  ...
  if (!archivePath) {
    console.error('Usage: node generate-release-manifest.mjs --archive <path> [--tag <vX.Y.Z>] [--commit <sha>] [--output-dir <dir>]');
    process.exit(1);
  }

  return { archivePath: resolve(process.cwd(), archivePath), tag, commitSha, outputDir };
}
...
function resolveCommitSha(providedSha) {
  ...
  throw new Error('Could not resolve valid 40-character commit SHA. Pass --commit <sha>.');
}
...
function assignRoles(relPath) {
  ...
  throw new Error(`Unrecognized release entry has no assigned role: '${relPath}'`);
}

function main() {
  const { archivePath, tag: userTag, commitSha: userSha, outputDir: userOutputDir } = parseArgs();
  ...
  let tag = userTag;
  if (!tag) {
    const match = archiveName.match(/^dam-hopper-(v[0-9]+\.[0-9]+\.[0-9]+)-fedora44-x86_64-systemd\.tar\.gz$/);
    if (match) {
      tag = match[1];
    } else {
      console.error(`Could not deduce release tag from archive name '${archiveName}'. Pass --tag <vX.Y.Z>.`);
      process.exit(1);
    }
  }
  ...
  const outDir = userOutputDir ? resolve(process.cwd(), userOutputDir) : dirname(archivePath);
  ...
  try {
    ...
    for (const line of tarListing) {
      ...
      if (isDir && (cleanPath === 'bin' || cleanPath === 'systemd' || cleanPath === 'sysusers.d')) {
        continue;
      }
