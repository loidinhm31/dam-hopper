...
function parseArgs() {
  ...
  if (!tag) {
    console.error(
      "Usage: node check-release-assets.mjs --tag <vX.Y.Z> [--dir <dir>] [--release-id <id>] [--repo <owner/repo>]",
    );
    process.exit(1);
  }
  ...
  for (const asset of assets) {
    ...
    if (localDigests && localDigests[asset.name]) {
      if (asset.size !== localDigests[asset.name].size) {
        throw new Error(
          `Remote asset '${asset.name}' size (${asset.size}) differs from local (${localDigests[asset.name].size})`,
        );
      }
    }
  }
